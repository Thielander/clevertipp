@if(isset($combinations2) && count($combinations2) > 0)
<p>{{ __('There are 1225 different combinations of 2. These are the 42 most common.') }}</p>
    <div class="row">

        @foreach($combinations2 as $kombination)
            <div class="col-lg-3 col-xl-2 mb-4">
                <div class="card bg-success text-white h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="me-3">
                                <div class="text-lg fw-bold font-monospace">  
                                    {{ __('Combination') }}:<br>
                                    {{ $kombination->num1 }} - {{ $kombination->num2 }}<br>
                                    {{ __('Count') }}: {{ $kombination->count }}<br>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        @endforeach
    </div>
@else
    <div class="alert alert-warning">{{ __('No data available') }}</div>
@endif
